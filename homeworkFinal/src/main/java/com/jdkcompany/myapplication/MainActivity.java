...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        contents.setText(readDiary(date.getText().toString()));
        //날짜 변경하는 부분
        date.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //날짜 부분 눌리면 다이얼로그 inflate해서 부름
        ...
        saveBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                File file = new File(dirPath+"/"+title);
                try {
    ...
    String readDiary(String fName){ // 파일 이름받아 있으면 받아와서 수정하기 / 없으면 일기없으니 새로저장
        ...
        try{
            inFs = new FileInputStream(dirPath +"/"+ fName+".txt");//정해진 디렉토리에 읽기
        ...
        switch (item.getItemId()){
            ...
            case  R.id.rmDiary:
                ...
                dlg.setPositiveButton("확인", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        File file = new File(dirPath + "/" + date.getText() + ".txt");
                        file.delete();
                        contents.setText("");
                    }
                });
